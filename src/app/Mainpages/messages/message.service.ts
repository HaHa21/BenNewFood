  ...
  private postsUpdated = new Subject<{ posts: Message[]; postCount: number }>();
   ...
   getMessages(postsPerPage: number, currentPage: number) {
     ...
     this.http
       .get<{ message: string; posts: any; maxPosts: number }>(
         "https://benfood.herokuapp.com/messages" + queryParams
       )
       .pipe(
         map(postData => {
           return {
    ...
    updateMessage(id: string, title: string, content: string){
      ...
      this.http
      .put("https://benfood.herokuapp.com/messages/" + id, messageData)
      .subscribe(response => {
     ...
     getMessage(id: string) {
       return this.http.get<{
         ...
         content: string;
         creator: string;
       }>("https://benfood.herokuapp.com/messages/" + id);
     }
   ...
   addMessage(content : string, title: string, creator: string){
       ...
       this.http.post<{ message: string; postId: string} >(
         "https://benfood.herokuapp.com/messages" , message
   ...
   deleteMessage(postId: string) {
       return this.http.delete("https://benfood.herokuapp.com/messages/" + postId);
     }
